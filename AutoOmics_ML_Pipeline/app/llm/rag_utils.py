...
class BiomarkerSynthesis(BaseModel):
    ...
    Core assessment fields (6, all required):
      interpretation, evidence_relation, evidence_tier, evidence_confidence,
      biomarker_potential, relevance_summary.

    Retrieval provenance fields (5, all Optional — default None):
      primary_evidence_source, supporting_sources, retrieval_notes,
      gene_normalization_notes, tier_basis.

    extra='forbid' rejects any key not declared in this model.
    """
    ...
    # ------------------------------------------------------------------
    # Retrieval provenance metadata (optional — backward compatible)
    # ------------------------------------------------------------------
    primary_evidence_source: Optional[Literal[
        "pubmed", "pmc", "ncbi_gene", "geo", "mixed", "inference"
    ]] = None
    supporting_sources:       Optional[List[str]] = None
    retrieval_notes:          Optional[str] = None
    gene_normalization_notes: Optional[str] = None

    # tier_basis refines evidence_tier for auditability.
    # Meaningful for Tier 1 and Tier 2 only — the key use case is distinguishing
    # Tier 2A (direct mechanistic evidence) from Tier 2B (network/PPI topology only).
    # Leave null for Tier 3 and Tier 4; those tiers are self-explanatory.
    tier_basis: Optional[Literal[
        "biomarker_direct",   # Tier 1: gene validated as biomarker in retrieved human studies
        "mechanistic_direct", # Tier 2A: direct mechanistic / association evidence in human data
        "network_systems",    # Tier 2B: gene appears in disease-paper network/PPI topology only —
                              # no direct wet-lab validation as biomarker or causal agent
    ]] = None
...
#              OR a directly related condition. "Directly related" is inferred from
#              the disease name and GEO_DATASET_CONTEXT — not a fixed definition.
#              (e.g. for an osteonecrosis study, AVN or bone ischemia papers qualify;
#               for a cancer study, the same cancer type in a different cohort qualifies)
#              These are examples only — apply the same logic to the active disease context.
#   indirect = no direct or directly-related disease paper; only biologically relevant
#              pathway or mechanism support exists in retrieved context
...
#   Tier 2 = human disease-specific evidence (mechanistic, network, or association);
#            use tier_basis to distinguish direct mechanistic/association (Tier 2A)
#            from network/PPI topology appearances only (Tier 2B)
...
_ITER_GUIDANCE = {
    ...
    1: (
        "pubmed_search AND ncbi_gene_search are both required this iteration.\n"
        "\n"
        "pubmed_search — disease-literal literature grounding:\n"
        "- Formulate ONE narrow PubMed query using the exact gene symbol and exact disease name.\n"
        "- Prefer: '<GENE> {{disease_context}}'.\n"
        "- Do NOT broaden with generic phrases like 'role in', 'bone health', 'signaling',\n"
        "  'mechanism', or wide OR clauses.\n"
        "- Do NOT use OR unless a synonym is explicitly present in already-retrieved tool output.\n"
        "- Goal: test for direct literature grounding first, not maximize recall.\n"
        "- If the exact disease query returns weak or no results, later iterations may broaden.\n"
        "\n"
        "ncbi_gene_search — gene normalization and synonym grounding:\n"
        "- Query must be the exact gene symbol only (e.g. 'IQGAP1').\n"
        "- Do NOT include disease context in this query.\n"
        "- Retrieves the official symbol, all known aliases/synonyms, and the functional\n"
        "  summary — use this context to interpret alias-based mentions in later retrieval.\n"
        "\n"
        "Call BOTH tools. Do not write an interpretation yet."
    ),
    2: (
        "Optional enrichment — pmc_fulltext_search and/or geo_search are available this iteration.\n"
        "\n"
        "pmc_fulltext_search — use when PubMed abstract evidence from Iter 1 is weak, missing,\n"
        "  or likely incomplete. PMC full-text recovers gene mentions that appear only in article\n"
        "  bodies (Methods / Results / Discussion), not in PubMed-indexed abstracts.\n"
        "  Formulate a query using the gene symbol and the disease context.\n"
        "\n"
        "geo_search — use when dataset or study-level context could strengthen the interpretation,\n"
        "  validate disease/source relevance, or help ground expression findings in public data.\n"
        "  Formulate a query using the gene and the disease or tissue context.\n"
        "\n"
        "Both tools may be called in a single iteration if both are warranted.\n"
        "If Iter 0–1 evidence is already sufficient for synthesis → skip tool calls."
    ),
    3: (
        "Optional targeted follow-up — choose ONE tool or skip.\n"
        "Pick whichever source produced the best unresolved lead in earlier iterations:\n"
        "\n"
        "- pubmed_fetch_by_id  : if a specific high-value PMID was cited in retrieved results\n"
        "                        and the full abstract has not yet been hydrated.\n"
        "- pubmed_search       : if additional literature grounding would strengthen the\n"
        "                        interpretation — use a refined or synonym-broadened query.\n"
        "- pmc_fulltext_search : if PMC returned a promising but incomplete signal in Iter 2\n"
        "                        → one more targeted full-text query.\n"
        "- geo_search          : if GEO surfaced a relevant dataset context gap in Iter 2\n"
        "                        → one more targeted dataset query.\n"
        "- uniprot_search      : if a biologically relevant secondary or interacting gene\n"
        "                        was identified → uniprot_search(secondary_gene).\n"
        "- ncbi_gene_search    : if alias/synonym confusion remains and a secondary gene\n"
        "                        or alias needs normalization.\n"
        "- wikipedia_search    : if background context is the only remaining gap.\n"
        "\n"
        "If context is already sufficient for a strong synthesis → do not call a tool."
    ),
    4: (
        ...
        "\n"
        "tier_basis — fill for Tier 1 and Tier 2 only; leave null for Tier 3 and Tier 4.\n"
        "  biomarker_direct   → Tier 1: gene validated as a biomarker in retrieved human studies.\n"
        "  mechanistic_direct → Tier 2A: direct mechanistic or association evidence in human data.\n"
        "  network_systems    → Tier 2B: gene appears in disease-paper network/PPI topology only\n"
        "                       (hub, co-expression, module) — NOT directly validated as biomarker\n"
        "                       or causal agent. Tier 2 is still correct; this flags the weaker basis.\n"
        "\n"
        "Provenance metadata fields:\n"
        "  primary_evidence_source: main source type that drove the interpretation\n"
        "    ('pubmed' | 'pmc' | 'ncbi_gene' | 'geo' | 'mixed' | 'inference').\n"
        "  supporting_sources: list of all source_type strings used across all iterations\n"
        "    (e.g. ['pubmed', 'ncbi_gene'] or ['pmc', 'uniprot', 'geo']).\n"
        "  retrieval_notes: one sentence on evidence provenance — note if key evidence\n"
        "    surfaced only in PMC full text rather than PubMed abstracts.\n"
        "  gene_normalization_notes: brief note on alias/synonym handling if relevant\n"
        "    (e.g. ncbi_gene_search revealed alias used in literature), or null."
    ),
}
...
_ITER_REASONING = {
    ...
    1: (
        "You must call pubmed_search AND ncbi_gene_search. "
        "For pubmed_search: use the narrowest exact disease query first — "
        "exact gene symbol + exact disease name. "
        "For ncbi_gene_search: use only the gene symbol, no disease context. "
        "Do not write an interpretation yet."
    ),
    2: (
        "Review PREVIOUS_ACTIONS and RETRIEVED_CONTEXT. "
        "If PubMed abstract evidence is weak, missing, or likely incomplete → call pmc_fulltext_search. "
        "If dataset or study-level context would strengthen interpretation → call geo_search. "
        "Both may be called if both are warranted. "
        "Skip if Iter 0–1 evidence is already sufficient for synthesis."
    ),
    3: (
        "Review PREVIOUS_ACTIONS. "
        "Pick ONE follow-up tool that deepens the most promising unresolved lead from earlier "
        "iterations — or skip entirely if context is already sufficient for synthesis."
    ),
...
- If this is the FINAL SYNTHESIS iteration (Iter 4), produce a structured response with these fields:
  ...
  evidence_relation   : "direct" | "indirect" | "inferred"
                        direct   = retrieved sources explicitly link this gene to {disease_context}
                                   OR a directly related condition (infer "directly related" from
                                   the disease name and GEO_DATASET_CONTEXT — not a fixed list)
                        indirect = retrieved evidence supports only a plausible mechanistic or
                                   pathway link; no direct or directly-related disease paper found
  ...
  evidence_tier       : "Tier 1" | "Tier 2" | "Tier 3" | "Tier 4"
                        ...
                        Tier 2 = human disease-specific evidence (mechanistic, network, or association);
                                 use tier_basis below to distinguish direct mechanistic support from
                                 network/PPI topology appearances only
  ...
  tier_basis          : "biomarker_direct" | "mechanistic_direct" | "network_systems" | null
                        Fill for Tier 1 and Tier 2 only — leave null for Tier 3 and Tier 4.
                        biomarker_direct   = Tier 1: gene validated as biomarker in retrieved human studies
                        mechanistic_direct = Tier 2A: direct mechanistic or association evidence in human data
                        network_systems    = Tier 2B: gene in disease-paper network/PPI topology only
                                            (hub, co-expression, module member) — NOT directly validated
                                            as a biomarker or causal agent in the active disease

  primary_evidence_source : "pubmed" | "pmc" | "ncbi_gene" | "geo" | "mixed" | "inference"
                           main source type that drove the final interpretation

  supporting_sources  : list of all source_type strings used across all iterations
                        (e.g. ["pubmed", "ncbi_gene"] or ["pmc", "uniprot", "geo"])

  retrieval_notes     : one sentence on evidence provenance — note especially if key evidence
                        surfaced only in PMC full text rather than PubMed abstracts

  gene_normalization_notes : brief note on alias/synonym handling if relevant
                             (e.g. ncbi_gene_search revealed a key alias that broadened
                             subsequent retrieval), or null if not applicable
...
- In Iter 1, the ncbi_gene_search query must be the exact gene symbol only — do not add disease context.
...
if __name__ == "__main__":
    ...
    _ITER_MOCK_TOOLS = {
        ...
        1: ["pubmed_search", "ncbi_gene_search"],
        2: ["pmc_fulltext_search", "geo_search"],
        3: ["pubmed_search", "pubmed_fetch_by_id", "pmc_fulltext_search",
            "geo_search", "uniprot_search", "ncbi_gene_search", "wikipedia_search"],
        4: [],
    }
