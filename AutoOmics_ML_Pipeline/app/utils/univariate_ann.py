...
# ---------------------------------------------------------------------------
_APP_DIR      = pathlib.Path(__file__).resolve().parent.parent   # AutoOmics_ML_Pipeline/app/
...
def parse_args():
    ...
    p.add_argument(
        "--model-type", type=str, default="univariate_ann",
        choices=["univariate_ann", "logistic", "svm_rbf", "all"],
        help="Model type  (default: univariate_ann). Use 'all' to run all three models with comparison outputs."
    )
...
if __name__ == "__main__":
    ...
    _MODEL_ALIAS = {"univariate_ann": "professor_ann"}
    models = (
        ["professor_ann", "logistic", "svm_rbf"]
        if args.model_type == "all"
        else [_MODEL_ALIAS.get(args.model_type, args.model_type)]
    )
