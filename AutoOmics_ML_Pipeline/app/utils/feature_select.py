...
_WEKA_SHARED_DIR = str(_PROJECT_ROOT / "data" / "femoral_head_necrosis" / "feature_selection")
...
def plot_statistical_vs_model_importance(
    ...
    if label_top_n > 0:
        ...
        for _, row in df.iterrows():
            if row["gene_symbol"] in top_stat | top_model:
                ax.text(
                    row["importance_j"],
                    row["hybrid_j"],
                    row["gene_symbol"],
...
if __name__ == "__main__":
    ...
    shared_outdir = _WEKA_SHARED_DIR
    ...
    # 5. Save full probe rankings (gene_rankings.csv) with all hybrid metrics — shared across branches
    rankings_path = os.path.join(shared_outdir, "gene_rankings.csv")
    annotation    = load_probe_annotation(SOFT_GZ)
    ...
    # 13. Gene-level summary — post-selection probe→gene grouping (shared across branches)
    gene_summary_path = os.path.join(shared_outdir, "gene_level_summary.csv")
    build_gene_level_summary(selected_df, fc_ranking, annotation, gene_summary_path)

    # 13b. Gene-level deduped rankings — one best probe per gene (shared across branches)
    gene_level_path = os.path.join(shared_outdir, "gene_level_rankings.csv")
